...
- Structured evaluator with strict JSON schema (score, reasoning, evidence tracking)
- Reflection memory across attempts (lesson + next strategy reuse)
- Full Reflexion loop (Actor → Evaluator → Reflector → retry)
- Adaptive max attempts based on question difficulty
- Automated benchmark reporting (JSON + Markdown)
- Data-driven discussion generation from metrics and failure modes
...
python run_benchmark.py --dataset data/hotpot_100_final_balanced.json --out-dir outputs/run
